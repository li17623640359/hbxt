...
//        }
        ...
        if ($accountType == 'email') {
            ...
            import('Three.AliNewSMS');


            $res = Db::name('user')->where(array('mobile' => $data['username']))->find();

            if ($res) {
                $this->success('该帐号已经被注册！');
                return;
            }

            Three\AliNewSMS::sendRegVerifyCode($data['username'], $code);
            cmf_verification_code_log($data['username'], $code);
    ...
    /**
     * send()的备份
     * @date: 2018年3月13日下午5:36:15
     * @author: Hhb
     */
    public function sendBak()
    {
        $validate = new Validate([
            'username' => 'require',
        ]);
